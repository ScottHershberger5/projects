...
def test_all_correct_answers():
    ...
    answers_queue.put(("New England", "New England"))
...
def test_all_wrong_answers():
    answers_queue: Queue[tuple[str, str]] = Queue()
    answers_queue.put(("New England", "Buffalo"))
...
def test_team_not_found():
    fake_teams = [{"name": "Patriots", "city": "New England", "coach": "Bill",
                   "owner": "Kraft", "stadium": "Gillette", "established": "1960"}]
...
# Test 4: get_team_data() returns a list when the API responds correctly
def test_get_team_data_returns_list():
    mock_response = MagicMock()
    mock_response.json.return_value = {"response": [{"name": "Patriots"}, {"name": "Bills"}]}
...
def test_team_name_variations(user_input):
    fake_teams = [{"name": "New England Patriots", "city": "Foxborough", "coach": "Bill",
                   "owner": "Kraft", "stadium": "Gillette", "established": "1960"}]

    with patch("builtins.input", side_effect=[user_input, "Foxborough", "Bill", "Kraft", "Gillette", "1960"]):
        result = questions(fake_teams)
